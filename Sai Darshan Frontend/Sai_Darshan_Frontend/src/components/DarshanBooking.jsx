...
function DarshanBooking() {
  ...
  useEffect(() => {
    if (selectedDate && selectedDarshan) {
      loadAvailableSlots();
    }
  }, [selectedDate, selectedDarshan]);

  const loadDarshanTypes = async () => {
    try {
      const types = await ApiService.getDarshanTypes();
  ...
  const loadAvailableSlots = async () => {
    try {
      ...
      // Map darshan type name to slot type
      const typeMapping = {
        'General Darshan': 'REGULAR',
        'VIP Darshan': 'VIP'
      };
      const slotType = typeMapping[selectedDarshan.name] || selectedDarshan.name.toUpperCase();
      const filteredSlots = slots.filter(slot => slot.type === slotType);
  ...
  const handleBooking = async () => {
    ...
    if (slot.availableSlots < numberOfPeople) {
      setError(`Only ${slot.availableSlots} seats available for this slot`);
      setLoading(false);
      return;
    }
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  >
                    ...
                    {availableSlots.map(slot => (
                      <option key={slot.id} value={slot.startTime}>
                        {slot.startTime} - {slot.endTime} ({slot.availableSlots} seats available)
                      </option>
                    ))}
                  </select>
                ) : (
              ...
              <div>
                ...
                >
                  {selectedSlot ? (
                    Array.from({length: Math.min(10, selectedSlot.availableSlots)}, (_, i) => i + 1).map(num => (
                      <option key={num} value={num}>{num} {num === 1 ? 'Person' : 'People'}</option>
                    ))
                  ) : (
                    <option value={1}>Select time first</option>
                  )}
                </select>
                {selectedSlot && (
                  <p className="text-sm text-gray-600 mt-1">
                    {selectedSlot.availableSlots} seats available
                  </p>
                )}
              </div>
