...
function AartiBooking() {
  ...
  }, [selectedAarti, selectedDate]);

  const loadCapacity = async () => {
    try {
      const aartiName = aartiTypes.find(a => a.id === selectedAarti)?.name;
      if (aartiName) {
        const cap = await ApiService.getAartiCapacity(aartiName, selectedDate);
        setCapacity(cap);
      }
    } catch (error) {
  ...
  const loadAartiTypes = async () => {
    try {
      ...
      // Transform backend data: "Name|Time|Price|Description|Duration|Guidelines"
      const transformedTypes = types.map((type) => {
        const [name, time, price, description, duration, guidelines] = type.split('|');
        return {
          id: name.toLowerCase().replace(/\s+/g, '_'),
          name: name,
          time: time || 'TBD',
          price: parseFloat(price) || 200,
          description: description || `Experience the divine ${name} ceremony`,
          duration: duration ? `${duration} minutes` : '30-45 minutes',
          guidelines: guidelines || 'Please arrive 15 minutes early'
        };
      });
  ...
  const handleFinalSubmit = async (e) => {
    ...
    // Validation
    if (!devoteeDetails.name || !devoteeDetails.age || !devoteeDetails.gender || !devoteeDetails.mobile || !devoteeDetails.email) {
    ...
    if (!acceptedTerms) {
      setError('Please accept the aarti guidelines to proceed');
      setLoading(false);
      return;
    }
          ...
          <div className="bg-gradient-to-r from-orange-50 to-yellow-50 rounded-xl p-6 mb-6 border-2 border-orange-200">
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                <span className="text-gray-600">Aarti Type:</span>
                <span className="font-semibold">{aartiTypes.find(a => a.id === selectedAarti)?.name}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-orange-600">{aartiTypes.find(a => a.id === selectedAarti)?.time}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold">₹{aartiTypes.find(a => a.id === selectedAarti)?.price}</span>
              </div>
              <div className="border-t-2 border-orange-300 pt-2 mt-2 flex justify-between items-center">
                <span className="text-lg font-bold text-gray-900">Total Amount:</span>
                <span className="text-2xl font-bold text-orange-600">₹{aartiTypes.find(a => a.id === selectedAarti)?.price * numberOfPeople}</span>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleFinalSubmit} className="space-y-6">
            ...
            <div className="bg-blue-50 border-2 border-blue-200 rounded-lg p-4">
              <label className="flex items-start cursor-pointer">
                ...
                <span className="text-sm text-gray-700">
                  I have read and agree to follow all <button type="button" onClick={() => setShowGuidelines(true)} className="text-blue-600 font-semibold underline hover:text-blue-800">aarti guidelines</button> including arriving 15 minutes early, maintaining silence, and following temple protocols.
                </span>
              </label>
            </div>
            ...
            <div className="flex gap-4 pt-4 relative z-10">
              <button
                ...
                className="flex-1 bg-gradient-to-r from-blue-600 to-blue-700 text-white py-3 rounded-lg font-semibold hover:from-blue-700 hover:to-blue-800 transition-all relative z-10"
              >
                Back
              </button>
              <button
                type="submit"
                disabled={loading}
                className="flex-1 bg-gradient-to-r from-orange-600 to-orange-700 text-white py-3 rounded-lg font-semibold hover:from-orange-700 hover:to-orange-800 transition-all disabled:opacity-50 disabled:cursor-not-allowed relative z-10"
              >
              ...
              <div>
                ...
                {capacity && (
                  <p className="text-sm text-blue-600 mt-1 font-medium">
                    {capacity.availableCapacity} / {capacity.maxCapacity} spots available
                  </p>
                )}
              </div>
